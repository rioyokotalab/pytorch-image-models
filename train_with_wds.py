...
parser.add_argument('--list-file', default='MVFractal_300k_shards.txt',
                    help='shards list using when training MVFractal')
...
def main():
    ...
    if args.log_wandb and args.rank == 0:
        if has_wandb:
            wandb.init(project=args.project_name, entity='yokota-vit', name=args.experiment, group='pretrain', config=args)
        else:
    ...
    if args.webdataset:
        ...
        # print0("Num of Batches:{}".format(num_batches))
        ...
        if num_aug_splits > 1:
            print0("Trying to use splitssss......")
        ...
        if args.trainshards is not None:
            ...
            list_fn = args.list_file

            with open(list_fn, "r") as fp:
                shard_fns = [re.sub("(^\\s+|\\s+$)", "", x) for x in fp.readlines() if not re.match("^\\s*$", x)]

...
def load_datasets(data_dir: Path, is_dry_run: bool):
    ...
    if not is_dry_run:
        ...
        datasets["ImageNet"] = torchvision.datasets.ImageNet(
            root=data_dir, split="val", transform=imagenet_transform
        )
        dl_dir = data_dir / "dead_leaves_small_scale"
        for child_dir in dl_dir.iterdir():
            datasets[child_dir.name] = torchvision.datasets.ImageFolder(
                root=child_dir, transform=imagenet_transform
            )
        fractal_dir = data_dir / "fractaldb_cat60_ins1000"
        datasets["FractalDBSmall"] = torchvision.datasets.ImageFolder(

    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            ...
            if args.fake_separated_loss_log:
                ...
                target_labels = torch.argmax(target, axis=1)
                ...
                if len(fake_target) == 0:
                    fake_loss = torch.zeros(1, dtype=torch.float32)
                if len(origin_target) == 0:
                    origin_loss = torch.zeros(1, dtype=torch.float32)
        ...
        if args.distributed:
            ...
            if args.fake_separated_loss_log:
                ...
                fake_local_sum_loss = torch.tensor([len(fake_target)*fake_loss.item()], dtype=torch.float32)
                dist.all_reduce(fake_local_sum_loss.data, op=dist.ReduceOp.SUM)
                fake_nums = torch.tensor([len(fake_target)], dtype=torch.int64)
                ...
                origin_local_sum_loss = torch.tensor([len(origin_target)*origin_loss.item()], dtype=torch.float32)
                dist.all_reduce(origin_local_sum_loss.data, op=dist.ReduceOp.SUM)
                origin_nums = torch.tensor([len(origin_target)], dtype=torch.int64)

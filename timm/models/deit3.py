...
from .vision_transformer import Mlp, PatchEmbed, checkpoint_filter_fn
...
default_cfgs = {
    'deit_tiny_patch16_LS': _cfg(url=''),
    'deit_small_patch16_LS': _cfg(url=''),
    'deit_base_patch16_LS': _cfg(url=''),
    'deit_large_patch16_LS': _cfg(url=''),
}
...
class VisionTransformerWithDeit3(nn.Module):
...
def _create_deit3(variant, pretrained=False, distilled=False, **kwargs):
    if kwargs.get('features_only', None):
        raise RuntimeError('features_only not implemented for Vision Transformer models.')
    model_cls = VisionTransformerWithDeit3
...
@register_model
def deit_tiny_patch16_LS(pretrained=False, **kwargs):
    model_kwargs = dict(patch_size=16, embed_dim=192, depth=12, num_heads=3, mlp_ratio=4, qkv_bias=True,
                        norm_layer=partial(nn.LayerNorm, eps=1e-6), block_layers=Layer_scale_init_Block, **kwargs)
    model = _create_deit3('deit_tiny_patch16_LS', pretrained=pretrained, **model_kwargs)
    return model
...
def deit_small_patch16_LS(pretrained=False, **kwargs):
    model_kwargs = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True,
                        norm_layer=partial(nn.LayerNorm, eps=1e-6), block_layers=Layer_scale_init_Block, **kwargs)
    model = _create_deit3('deit_small_patch16_LS', pretrained=pretrained, **model_kwargs)
    return model
    

@register_model
def deit_base_patch16_LS(pretrained=False, **kwargs):
    model_kwargs = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
                        norm_layer=partial(nn.LayerNorm, eps=1e-6), block_layers=Layer_scale_init_Block, **kwargs)
    model = _create_deit3('deit_base_patch16_LS', pretrained=pretrained, **model_kwargs)
    return model    


@register_model
def deit_large_patch16_LS(pretrained=False, **kwargs):
    model_kwargs = dict(patch_size=16, embed_dim=1024, depth=24, num_heads=16, mlp_ratio=4, qkv_bias=True,
                        norm_layer=partial(nn.LayerNorm, eps=1e-6), block_layers=Layer_scale_init_Block, **kwargs)
    model = _create_deit3('deit_large_patch16_LS', pretrained=pretrained, **model_kwargs)
    return model

...
class KFAC(optim.Optimizer):
    def __init__(self,
        ...
        defaults = dict(lr=lr, momentum=momentum, damping=damping*damping_scale,
        ...
        self.damping_factor = 2 * math.log10(damping_scale) / damping_warmup_steps
    ...
    def adjust_damping(self, damping):
        return (1 - self.damping_factor) * damping + self.damping_factor * self.damping_min
    ...
    def _update_inv(self, m):
        ...
        self.d_a[m], self.Q_a[m] = torch.linalg.eigh(self.m_aa[m]) # torch.symeig(self.m_aa[m], eigenvectors=True)
        self.d_g[m], self.Q_g[m] = torch.linalg.eigh(self.m_gg[m]) # torch.symeig(self.m_gg[m], eigenvectors=True)
